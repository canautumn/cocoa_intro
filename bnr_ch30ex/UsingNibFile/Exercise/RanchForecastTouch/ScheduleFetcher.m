...
@property (strong) ScheduleFetcherResultBlock resultBlock;
...
- (void)fetchClassesWithBlock:(ScheduleFetcherResultBlock)theBlock {
    ...
    self.resultBlock = [theBlock copy];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    if (!success) {
        self.resultBlock(nil, [parser parserError]);
    } else {
        NSArray *output = [self.classes copy];
        self.resultBlock(output, nil);
    }

...
- (instancetype)init {
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handleColorChange:) name:ColorChangeNotification object:nil];
    }
    return self;
}
...
- (void)handleColorChange:(NSNotification *)note {
    ...
    [_tableView setBackgroundColor:[[note userInfo] objectForKey:@"color"]];
}
...
- (void)insertObject:(Person *)p inEmployeesAtIndex:(NSUInteger)index {
    ...
    if (![undo isUndoing]) {
        [undo setActionName:NSLocalizedString(@"ADD_PERSON", @"Add Person")];
    }
...
- (void)removeObjectFromEmployeesAtIndex:(NSUInteger)index {
    ...
    if (![undo isUndoing]) {
        [undo setActionName:NSLocalizedString(@"REMOVE_PERSON", @"Remove Person")];
    }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    [undo setActionName:NSLocalizedString(@"EDIT", @"Edit")]; // why not check "undoing"?
}
...
- (IBAction)removeEmployee:(id)sender {
    ...
    [alert addButtonWithTitle:NSLocalizedString(@"KEEPNORAISE", @"Keep, but no raise")];
    [alert setMessageText:NSLocalizedString(@"REMOVE_MSG", @"Do you really want to remove these people?")];
    [alert setInformativeText:[NSString stringWithFormat:NSLocalizedString(@"REMOVE_INF", @"%lu people will be removed."), [selectedPeople count]]];
    [alert beginSheetModalForWindow:[_tableView window] completionHandler:^(NSModalResponse returnCode) {
        ...
        if (returnCode == NSAlertFirstButtonReturn) {
            ...
            for (Person * p in [_employeeController selectedObjects]) {
                p.expectedRaise = 0.0;
            }
        }
    }];
...
- (NSPrintOperation *)printOperationWithSettings:(NSDictionary *)printSettings error:(NSError *__autoreleasing *)outError {
    PeopleView *view = [[PeopleView alloc] initWithPeople:_employees];

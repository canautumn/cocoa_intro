...
- (void)windowControllerDidLoadNib:(NSWindowController *)aController {
    ...
    for (int i = 0; i < self.viewControllers.count; ++i) {
        ...
        NSMenuItem *mi = [[NSMenuItem alloc] initWithTitle:vc.description action:@selector(changeViewController:) keyEquivalent:@""];
        mi.tag = i;
        [menu addItem:mi];
    }
    
    NSMenuItem *mi = [[NSMenuItem alloc] initWithTitle:@"another test" action:@selector(anotherTest) keyEquivalent:@""];
    [menu addItem:mi];
    
//    [self.box.contentView frame].size = [[[self.viewControllers objectAtIndex:0] view] frame].size;
...
- (void)anotherTest {
    TestTestView *test = [[TestTestView alloc] init];
    self.box.contentView = test.view;
}
...
//    NSLog(@"%f, %f", currentSize.height, currentSize.width);
